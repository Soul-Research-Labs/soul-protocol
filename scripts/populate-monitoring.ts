    ...
    if (fs.existsSync(filePath)) {
      ...
      if (data.contracts) {
        ...
        for (const [name, info] of Object.entries(data.contracts)) {
          result[name] =
            typeof info === "string" ? info : (info as any).address;
        }
        return result;
      }
      return data;
...
function populateDefenderConfig(
  ...
  console.log(
    `✅ Defender config updated: ${addressesPopulated} addresses populated`,
  );
}

function populateTenderlyConfig(
  deployments: DeploymentInfo,
  network: string,
): void {
  const configPath = path.join(
  ...
  if (config.alerts) {
    for (const alert of config.alerts) {
      if (alert.contracts && alert.contracts.length === 0) {
        const contracts = Object.entries(deployments).map(
          ([name, address]) => ({
            name,
            address,
            network:
  ...
  fs.writeFileSync(configPath, JSON.stringify(config, null, 2));
  console.log(
    `✅ Tenderly config updated: ${addressesPopulated} addresses populated`,
  );
}
...
function generateAlertChannels(network: string): void {
  ...
  // Production alerting must never silently fall back to placeholder values.
  const requiredEnvVars: Record<string, string | undefined> = {
    PAGERDUTY_ROUTING_KEY: process.env.PAGERDUTY_ROUTING_KEY,
    TELEGRAM_CHAT_ID: process.env.TELEGRAM_CHAT_ID,
    TELEGRAM_BOT_TOKEN: process.env.TELEGRAM_BOT_TOKEN,
    SLACK_CRITICAL_WEBHOOK: process.env.SLACK_CRITICAL_WEBHOOK,
    SLACK_HIGH_WEBHOOK: process.env.SLACK_HIGH_WEBHOOK,
    SLACK_MEDIUM_WEBHOOK: process.env.SLACK_MEDIUM_WEBHOOK,
    SLACK_LOW_WEBHOOK: process.env.SLACK_LOW_WEBHOOK,
  };
  ...
  if (missing.length > 0) {
    throw new Error(
      `Missing required environment variables for monitoring alert channels: ${missing.join(", ")}. ` +
        `Set these in your .env file or CI secrets before running this script.`,
    );
  }
  ...
  const channels = {
    ...
    channels: {
      ...
      high: {
        slack: {
          webhookUrl: process.env.SLACK_HIGH_WEBHOOK,
      ...
      medium: {
        slack: {
          webhookUrl: process.env.SLACK_MEDIUM_WEBHOOK,
          channel: "#zaseon-monitoring",
        },
      },
      low: {
        slack: {
          webhookUrl: process.env.SLACK_LOW_WEBHOOK,
          channel: "#zaseon-monitoring",
        },
      },
    },
...
console.log(
  `Loaded ${Object.keys(deployments).length} deployed contract addresses\n`,
);
...
console.log(
  "  1. Set notification environment variables (PAGERDUTY_ROUTING_KEY, etc.)",
);
