...
describe("Noir Verifier Adapters", () => {
  ...
  beforeEach(async () => {
    ...
    mockVerifier = await (hre as any).viem.deployContract(
      "MockNoirVerifier" as any,
    );

    // 2. Deploy the adapter pointing to the mock
    adapter = await (hre as any).viem.deployContract("PolicyVerifierAdapter", [
  ...
  it("should format and delegate verification to Noir verifier", async () => {
    ...
    const publicInputs =
      "0x" + lenHex + signals.map((s) => s.slice(2)).join("");

    // Call the adapter
    const result = await adapter.read.verify([
      circuitHash,
      proof,
  ...
  it("should revert if signal is out of field range", async () => {
    ...
    await expect(
      adapter.read.verify([circuitHash, proof, publicInputs as `0x${string}`]),
