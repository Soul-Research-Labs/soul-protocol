/**
 * Certora Verification Spec: SolanaBridgeAdapter
 *
 * Verifies critical invariants for the Soul Protocol <-> Solana bridge:
 * - Nonce monotonicity
...
using SolanaBridgeAdapter as bridge;
...
ghost mapping(bytes32 => bool) ghostUsedTxSignatures;
...
/// @title Lamports per SOL is always 1 billion
invariant lamportsPerSolConstant()
    bridge.LAMPORTS_PER_SOL() == 1000000000;

/// @title Min deposit is always 0.1 SOL (100M lamports)
invariant minDepositConstant()
    bridge.MIN_DEPOSIT_LAMPORTS() == 100000000;
...
/// @title Once a Solana tx signature is used, it stays used forever
rule replayProtectionPermanence(bytes32 txSig, method f) {
    bool usedBefore = bridge.usedSolanaTxSignatures(txSig);
    ...
    bool usedAfter = bridge.usedSolanaTxSignatures(txSig);
    assert usedAfter == true, "Used tx signature was reset";
}
...
rule onlyOperatorCanCompleteDeposit(env e) {
    ...
    bridge.completeSOLDeposit@withrevert(e, depositId);
...
rule pauseBlocksDeposits(env e) {
    ...
    bytes32 txSig; bytes32 sender; address recipient;
    uint256 amount; uint256 slot;
    ISolanaBridgeAdapter.SolanaMerkleProof proof;
    ISolanaBridgeAdapter.GuardianAttestation[] attestations;

    bridge.initiateSOLDeposit@withrevert(
        e, txSig, sender, recipient, amount, slot, proof, attestations
    );
...
/*//////////////////////////////////////////////////////////////
            RULE: SLOT INDEX MONOTONICITY
//////////////////////////////////////////////////////////////*/

/// @title Latest slot index never decreases
rule latestSlotNeverDecreases(method f) {
    uint256 slotBefore = bridge.latestSlot();

    env e;
    calldataarg args;
    f(e, args);

    uint256 slotAfter = bridge.latestSlot();
    assert slotAfter >= slotBefore, "Latest slot decreased";
}
