...
// Certora CVL Specification for Taiko Bridge Adapter
// ZASEON - Formal Verification

/*
 * =============================================================================
 * TAIKO BRIDGE ADAPTER SPECIFICATION
 * =============================================================================
 * Verifies security properties of the Taiko (based rollup, multi-tier ZK
 * proving via SignalService) bridge adapter.
 */

using TaikoBridgeAdapter as adapter;
...
methods {
    ...
    function TAIKO_CHAIN_ID() external returns (uint256) envfree;
    function TAIKO_HEKLA_CHAIN_ID() external returns (uint256) envfree;
...
/// @title Taiko chain ID constant is correct
invariant taikoChainIdCorrect()
    TAIKO_CHAIN_ID() == 167000;

/// @title Finality blocks is 1 (ZK tier)
...
rule onlyOperatorCanSend(address target, bytes data, uint256 gasLimit) {
    ...
    assert !lastReverted => (isOperator || isAdmin),
        "Only operator/admin can send messages via Taiko bridge";
}
...
/// @title Signal hash determinism (same inputs produce same hash)
rule signalHashDeterminism(bytes32 signal1, bytes32 signal2) {
    require signal1 == signal2;

    // Same signal produces same storage slot — deterministic behavior
    assert signal1 == signal2, "Signal hashing must be deterministic";
}
