...
// Certora CVL Specification for Manta Pacific Bridge Adapter
// ZASEON - Formal Verification

/*
 * =============================================================================
 * MANTA PACIFIC BRIDGE ADAPTER SPECIFICATION
 * =============================================================================
 * Verifies security properties of the Manta Pacific (Polygon CDK + Celestia DA)
 * bridge adapter. ZK proof finality with Sparse Merkle Tree proofs.
 */

using MantaPacificBridgeAdapter as adapter;
...
methods {
    ...
    function MANTA_PACIFIC_CHAIN_ID() external returns (uint256) envfree;
    function MANTA_SEPOLIA_CHAIN_ID() external returns (uint256) envfree;
    function FINALITY_BLOCKS() external returns (uint256) envfree;
    function MAX_PROOF_SIZE() external returns (uint256) envfree;
    function NETWORK_ID_MAINNET() external returns (uint256) envfree;
    function NETWORK_ID_MANTA() external returns (uint256) envfree;
...
ghost mapping(bytes32 => bool) ghostClaimedDeposits;
...
hook Sstore claimedDeposits[KEY bytes32 leafHash] bool isClaimed {
    ghostClaimedDeposits[leafHash] = isClaimed;
}
...
/// @title Manta Pacific chain ID constant is correct
invariant mantaPacificChainIdCorrect()
    MANTA_PACIFIC_CHAIN_ID() == 169;

/// @title Finality blocks is 1 (ZK proof finality)
...
/// @title Network IDs are correct (CDK convention)
invariant networkIdsCorrect()
    NETWORK_ID_MAINNET() == 0 && NETWORK_ID_MANTA() == 1;
...
rule onlyOperatorCanSend(address target, bytes data, uint256 gasLimit) {
    ...
    assert !lastReverted => (isOperator || isAdmin),
        "Only operator/admin can send messages via Manta Pacific bridge";
}
...
/// @title Nullifier uniqueness (CDK deposits)
rule nullifierUniqueness(uint32 depositCount1, uint32 depositCount2, uint32 originNetwork) {
    ...
    bytes32 nf1 = keccak256(abi.encodePacked(depositCount1, originNetwork, "MANTA_NULLIFIER"));
    bytes32 nf2 = keccak256(abi.encodePacked(depositCount2, originNetwork, "MANTA_NULLIFIER"));

    assert nf1 != nf2, "Different deposits must have different nullifiers";
}

/// @title Cross-domain nullifier determinism
rule crossDomainNullifierDeterminism(bytes32 mantaNullifier, bytes32 domain) {
    bytes32 zaseonNf1 = keccak256(abi.encodePacked(mantaNullifier, domain, "MANTA2Zaseon"));
    bytes32 zaseonNf2 = keccak256(abi.encodePacked(mantaNullifier, domain, "MANTA2Zaseon"));
