...
rule crossDomainNullifierDeterminism(bytes32 opNullifier, bytes32 domain) {
    bytes32 zaseonNf1 = keccak256(abi.encodePacked(opNullifier, domain, "OP2Zaseon"));
    bytes32 zaseonNf2 = keccak256(abi.encodePacked(opNullifier, domain, "OP2Zaseon"));
    
    assert zaseonNf1 == zaseonNf2, "Cross-domain nullifier must be deterministic";
}

/// @title Cross-domain direction matters
/// @notice OP->Zaseon differs from Zaseon->OP
