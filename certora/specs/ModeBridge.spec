...
// Certora CVL Specification for Mode Bridge Adapter
// ZASEON - Formal Verification

/*
 * =============================================================================
 * MODE BRIDGE ADAPTER SPECIFICATION
 * =============================================================================
 * Verifies security properties of the Mode (OP Stack Bedrock + Sequencer Fee
 * Sharing) bridge adapter. ~7 day optimistic challenge window.
 */

using ModeBridgeAdapter as adapter;
...
methods {
    ...
    function MODE_CHAIN_ID() external returns (uint256) envfree;
    function MODE_SEPOLIA_CHAIN_ID() external returns (uint256) envfree;
...
/// @title Mode chain ID constant is correct
invariant modeChainIdCorrect()
    MODE_CHAIN_ID() == 34443;
...
rule onlyOperatorCanSend(address target, bytes data, uint256 gasLimit) {
    ...
    assert !lastReverted => (isOperator || isAdmin),
        "Only operator/admin can send messages via Mode bridge";
}
