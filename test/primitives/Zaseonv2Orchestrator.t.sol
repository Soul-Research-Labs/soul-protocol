...
import "../../contracts/primitives/Zaseonv2Orchestrator.sol";
...
contract Zaseonv2OrchestratorTest is Test {
    Zaseonv2Orchestrator public orch;
    MockPC3 public pc3;
    MockPBP public pbp;
    ...
    function test_registerContainerInDomain_revertsZeroContainerId() public {
        vm.prank(operator);
        vm.expectRevert(Zaseonv2Orchestrator.InvalidContainerId.selector);
    ...
    function test_registerContainerInDomain_revertsZeroDomainId() public {
        vm.prank(operator);
        vm.expectRevert(Zaseonv2Orchestrator.InvalidDomainId.selector);
        orch.registerContainerInDomain(
            bytes32(uint256(1)),
            bytes32(uint256(2)),
            bytes32(uint256(3)),
    ...
    function test_createPolicyBoundCommitment_revertsInvalidPolicy() public {
        vm.prank(operator);
        vm.expectRevert(Zaseonv2Orchestrator.InvalidPolicyId.selector);
    ...
    function test_completeCoordinatedTransition_revertsNotFound() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                Zaseonv2Orchestrator.TransitionNotFound.selector,
                fakeTid
            )
        );
        orch.completeCoordinatedTransition(
        ...
        public
    {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                Zaseonv2Orchestrator.TransitionAlreadyComplete.selector,
                tid
            )
        );
        orch.completeCoordinatedTransition(
