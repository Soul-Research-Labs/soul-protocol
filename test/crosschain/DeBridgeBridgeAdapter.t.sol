...
import {DeBridgeBridgeAdapter, IDeBridgeGate, IDeBridgeValidator} from "../../contracts/crosschain/DeBridgeBridgeAdapter.sol";
import "@openzeppelin/contracts/token/ERC20/ERC20.sol";

contract MockDeBridgeGate {
    bytes32 public nextSubmissionId = keccak256("debridge-msg-1");
    ...
    function send(
        ...
        require(!shouldRevert, "MockDeBridgeGate: reverted");
    ...
    function setNextSubmissionId(bytes32 _id) external {
        nextSubmissionId = _id;
    }
...
contract MockDeBridgeValidator {
    bool public shouldVerify = true;
    bytes32 public validatorSetHash = keccak256("debridge-validators-1");
...
contract MockERC20DeBridge is ERC20 {
...
contract DeBridgeBridgeAdapterTest is Test {
    DeBridgeBridgeAdapter public adapter;
    MockDeBridgeGate public bridge;
    MockDeBridgeValidator public verifier;
    MockERC20DeBridge public token;
    ...
    function setUp() public {
        bridge = new MockDeBridgeGate();
        verifier = new MockDeBridgeValidator();
        token = new MockERC20DeBridge();
        adapter = new DeBridgeBridgeAdapter(
            address(bridge),
            address(verifier),
            admin
        );
    ...
    function test_constructor_setsBridge() public view {
        assertEq(address(adapter.deBridgeGate()), address(bridge));
    }

    function test_constructor_setsVerifier() public view {
        assertEq(address(adapter.deBridgeValidator()), address(verifier));
    }
    ...
    function test_constructor_revertsZeroBridge() public {
        vm.expectRevert(DeBridgeBridgeAdapter.InvalidBridge.selector);
        new DeBridgeBridgeAdapter(address(0), address(verifier), admin);
    }

    function test_constructor_revertsZeroVerifier() public {
        vm.expectRevert(DeBridgeBridgeAdapter.InvalidVerifier.selector);
        new DeBridgeBridgeAdapter(address(bridge), address(0), admin);
    }

    function test_constructor_revertsZeroAdmin() public {
        vm.expectRevert(DeBridgeBridgeAdapter.InvalidTarget.selector);
        new DeBridgeBridgeAdapter(
            address(bridge),
            address(verifier),
            address(0)
        );
    ...
    function test_constants() public view {
        assertEq(adapter.DEBRIDGE_PROTOCOL_ID(), 30_100);
    ...
    function test_chainId() public view {
        assertEq(adapter.chainId(), 30_100);
    }

    function test_chainName() public view {
        assertEq(adapter.chainName(), "deBridge");
    }
    ...
    function test_sendMessage_revertsEmptyPayload() public {
        vm.prank(operator);
        vm.expectRevert(DeBridgeBridgeAdapter.InvalidPayload.selector);
    ...
    function test_receiveMessage_revertsInvalidProof() public {
        ...
        vm.prank(relayer);
        vm.expectRevert(DeBridgeBridgeAdapter.InvalidProof.selector);
    ...
    function test_receiveMessage_revertsDuplicateNullifier() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                DeBridgeBridgeAdapter.NullifierAlreadyUsed.selector,
                keccak256("null-dup")
            )
        );
        adapter.receiveMessage(proof, inputs, hex"beef");
    ...
    function test_setBridgeFee_revertsAboveMax() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                DeBridgeBridgeAdapter.FeeTooHigh.selector,
                101
            )
        );
