...
contract ScrollBridgeAdapterTest is Test {
    ...
    function setUp() public {
        adapter = new ScrollBridgeAdapter(admin);
        vm.startPrank(admin);
    ...
    function test_ConfigureScroll() public {
        vm.prank(operator);
        adapter.configureScroll(
            534352, // Scroll chain ID
            address(0x1), // l1Messenger
            address(0x2), // l1GatewayRouter
            address(0x3), // l1MessageQueue
            address(0x4) // rollup
        );
    }

    function test_ConfigureScroll_RevertNonOperator() public {
        vm.prank(user);
        vm.expectRevert();
        adapter.configureScroll(
            534352,
    ...
    function test_ConfigureScroll_RevertZeroAddress() public {
        vm.prank(operator);
        vm.expectRevert();
        adapter.configureScroll(
            534352,
    ...
    function test_RegisterWithdrawal_RevertNonExecutor() public {
        vm.prank(user);
        vm.expectRevert();
        adapter.registerWithdrawal(user, user, address(0), 1 ether, 1);
    }
    ...
    function test_EstimateFee_Reverts() public {
        vm.expectRevert("Use Scroll-specific fee estimation");
