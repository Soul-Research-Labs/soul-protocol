...
import {TronBridgeAdapter, ITronBridge, ITronProofVerifier} from "../../contracts/crosschain/TronBridgeAdapter.sol";
import "@openzeppelin/contracts/token/ERC20/ERC20.sol";

contract MockTronBridge {
    bytes32 public nextMessageId = keccak256("tron-msg-1");
    uint256 public relayFee = 0.001 ether;
    uint256 public verifiedBlock = 60_000_000;
    ...
    function relayToTron(
        ...
        require(!shouldRevert, "MockTronBridge: reverted");
...
contract MockTronProofVerifier {
    bool public shouldVerify = true;
    bytes32 public committeeHash = keccak256("tron-committee-1");
...
contract MockERC20Tron is ERC20 {
...
contract TronBridgeAdapterTest is Test {
    TronBridgeAdapter public adapter;
    MockTronBridge public bridge;
    MockTronProofVerifier public verifier;
    MockERC20Tron public token;
    ...
    function setUp() public {
        bridge = new MockTronBridge();
        verifier = new MockTronProofVerifier();
        token = new MockERC20Tron();
        adapter = new TronBridgeAdapter(
            address(bridge),
            address(verifier),
            admin
        );
    ...
    function test_constructor_setsBridge() public view {
        assertEq(address(adapter.tronBridge()), address(bridge));
    }
    ...
    function test_constructor_revertsZeroBridge() public {
        vm.expectRevert(TronBridgeAdapter.InvalidBridge.selector);
        new TronBridgeAdapter(address(0), address(verifier), admin);
    }

    function test_constructor_revertsZeroVerifier() public {
        vm.expectRevert(TronBridgeAdapter.InvalidVerifier.selector);
        new TronBridgeAdapter(address(bridge), address(0), admin);
    }

    function test_constructor_revertsZeroAdmin() public {
        vm.expectRevert(TronBridgeAdapter.InvalidTarget.selector);
        new TronBridgeAdapter(address(bridge), address(verifier), address(0));
    }

    function test_constants() public view {
        assertEq(adapter.TRON_CHAIN_ID(), 20_100);
    ...
    function test_chainId() public view {
        assertEq(adapter.chainId(), 20_100);
    }

    function test_chainName() public view {
        assertEq(adapter.chainName(), "Tron");
    }
    ...
    function test_sendMessage_revertsEmptyPayload() public {
        vm.prank(operator);
        vm.expectRevert(TronBridgeAdapter.InvalidPayload.selector);
    ...
    function test_receiveMessage_revertsInvalidProof() public {
        ...
        vm.prank(relayer);
        vm.expectRevert(TronBridgeAdapter.InvalidProof.selector);
    ...
    function test_receiveMessage_revertsDuplicateNullifier() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                TronBridgeAdapter.NullifierAlreadyUsed.selector,
                keccak256("null-dup")
            )
        );
        adapter.receiveMessage(proof, inputs, hex"beef");
    ...
    function test_setBridgeFee_revertsAboveMax() public {
        vm.prank(admin);
        vm.expectRevert(
            abi.encodeWithSelector(TronBridgeAdapter.FeeTooHigh.selector, 101)
        );
    ...
    function testFuzz_setBridgeFee_bounds(uint256 fee) public {
        if (fee <= 100) {
            ...
            vm.expectRevert(
                abi.encodeWithSelector(
                    TronBridgeAdapter.FeeTooHigh.selector,
                    fee
                )
            );
            adapter.setBridgeFee(fee);
