...
import "../../contracts/crosschain/ModeBridgeAdapter.sol";

/// @dev Mock CrossDomainMessenger for Mode
contract MockCrossDomainMessengerMode {
    function sendMessage(address, bytes memory, uint32) external payable {}
}

/// @dev Mock Mode OptimismPortal
contract MockModePortal {
    fallback() external payable {}

    receive() external payable {}
}

/// @dev Mock L2OutputOracle for Mode
contract MockOutputOracleMode {
    function getL2Output(uint256) external pure returns (bytes32) {
        return keccak256("mode_output");
    }
}

contract ModeBridgeAdapterTest is Test {
    ModeBridgeAdapter public adapter;
    MockCrossDomainMessengerMode public mockMessenger;
    MockModePortal public mockPortal;
    MockOutputOracleMode public mockOracle;
    ...
    function setUp() public {
        mockMessenger = new MockCrossDomainMessengerMode();
        mockPortal = new MockModePortal();
        mockOracle = new MockOutputOracleMode();

        adapter = new ModeBridgeAdapter(
    ...
    function test_constructor_setsStorage() public view {
        ...
        assertEq(adapter.modePortal(), address(mockPortal));
    ...
    function test_constants() public view {
        assertEq(adapter.MODE_CHAIN_ID(), 34443);
        assertEq(adapter.MODE_SEPOLIA_CHAIN_ID(), 919);
    ...
    function test_chainId() public view {
        assertEq(adapter.chainId(), 34443);
    }

    function test_chainName() public view {
        assertEq(adapter.chainName(), "Mode");
    }
    ...
    function test_configureModeBridge() public {
        address newMsg = makeAddr("newMsg");
        address newPortal = makeAddr("newPortal");
        address newOracle = makeAddr("newOracle");

        vm.prank(operator);
        adapter.configureModeBridge(newMsg, newPortal, newOracle);

        assertEq(adapter.crossDomainMessenger(), newMsg);
        assertEq(adapter.modePortal(), newPortal);
        assertEq(adapter.outputOracle(), newOracle);
    }

    function test_configureModeBridge_revert_zeroMessenger() public {
        vm.prank(operator);
        vm.expectRevert("Invalid cross domain messenger");
        adapter.configureModeBridge(
            address(0),
            address(mockPortal),
            address(mockOracle)
        );
