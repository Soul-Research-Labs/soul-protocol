...
import {StellarBridgeAdapter, IStellarBridge, IStellarSCPVerifier} from "../../contracts/crosschain/StellarBridgeAdapter.sol";
import "@openzeppelin/contracts/token/ERC20/ERC20.sol";

contract MockStellarBridge {
    bytes32 public nextMessageId = keccak256("stellar-msg-1");
    uint256 public relayFee = 0.001 ether;
    uint256 public verifiedLedger = 50_000_000;
    ...
    function relayToStellar(
        ...
        require(!shouldRevert, "MockStellarBridge: reverted");
    ...
    function latestVerifiedLedger() external view returns (uint256) {
...
contract MockStellarSCPVerifier {
    bool public shouldVerify = true;
    bytes32 public quorumHash = keccak256("stellar-quorum-1");
...
contract MockERC20Stellar is ERC20 {
...
contract StellarBridgeAdapterTest is Test {
    StellarBridgeAdapter public adapter;
    MockStellarBridge public bridge;
    MockStellarSCPVerifier public verifier;
    MockERC20Stellar public token;
    ...
    function setUp() public {
        bridge = new MockStellarBridge();
        verifier = new MockStellarSCPVerifier();
        token = new MockERC20Stellar();
        adapter = new StellarBridgeAdapter(
            address(bridge),
            address(verifier),
            admin
        );
    ...
    function test_constructor_setsBridge() public view {
        assertEq(address(adapter.stellarBridge()), address(bridge));
    }

    function test_constructor_setsVerifier() public view {
        assertEq(address(adapter.scpVerifier()), address(verifier));
    }
    ...
    function test_constructor_revertsZeroBridge() public {
        vm.expectRevert(StellarBridgeAdapter.InvalidBridge.selector);
        new StellarBridgeAdapter(address(0), address(verifier), admin);
    }

    function test_constructor_revertsZeroVerifier() public {
        vm.expectRevert(StellarBridgeAdapter.InvalidVerifier.selector);
        new StellarBridgeAdapter(address(bridge), address(0), admin);
    }

    function test_constructor_revertsZeroAdmin() public {
        vm.expectRevert(StellarBridgeAdapter.InvalidTarget.selector);
        new StellarBridgeAdapter(
            address(bridge),
            address(verifier),
            address(0)
        );
    ...
    function test_constants() public view {
        assertEq(adapter.STELLAR_CHAIN_ID(), 27_100);
    ...
    function test_chainId() public view {
        assertEq(adapter.chainId(), 27_100);
    }

    function test_chainName() public view {
        assertEq(adapter.chainName(), "Stellar");
    }
    ...
    function test_sendMessage_revertsEmptyPayload() public {
        vm.prank(operator);
        vm.expectRevert(StellarBridgeAdapter.InvalidPayload.selector);
    ...
    function test_receiveMessage_success() public {
        ...
        inputs[0] = uint256(keccak256("quorum"));
    ...
    function test_receiveMessage_revertsInvalidProof() public {
        ...
        vm.prank(relayer);
        vm.expectRevert(StellarBridgeAdapter.InvalidProof.selector);
    ...
    function test_receiveMessage_revertsDuplicateNullifier() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                StellarBridgeAdapter.NullifierAlreadyUsed.selector,
                keccak256("null-dup")
            )
        );
        adapter.receiveMessage(proof, inputs, hex"beef");
    ...
    function test_setBridgeFee_revertsAboveMax() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                StellarBridgeAdapter.FeeTooHigh.selector,
                101
            )
        );
