...
import {AlgorandBridgeAdapter, IAlgorandBridge, IAlgorandStateProofVerifier} from "../../contracts/crosschain/AlgorandBridgeAdapter.sol";
import "@openzeppelin/contracts/token/ERC20/ERC20.sol";

contract MockAlgorandBridge {
    bytes32 public nextMessageId = keccak256("algorand-msg-1");
    uint256 public relayFee = 0.001 ether;
    uint256 public verifiedRound = 35_000_000;
    ...
    function relayToAlgorand(
        ...
        require(!shouldRevert, "MockAlgorandBridge: reverted");
    ...
    function latestVerifiedRound() external view returns (uint256) {
...
contract MockAlgorandStateProofVerifier {
    bool public shouldVerify = true;
    bytes32 public participationHash = keccak256("algorand-participation-1");
...
contract MockERC20Algorand is ERC20 {
...
contract AlgorandBridgeAdapterTest is Test {
    AlgorandBridgeAdapter public adapter;
    MockAlgorandBridge public bridge;
    MockAlgorandStateProofVerifier public verifier;
    MockERC20Algorand public token;
    ...
    function setUp() public {
        bridge = new MockAlgorandBridge();
        verifier = new MockAlgorandStateProofVerifier();
        token = new MockERC20Algorand();
        adapter = new AlgorandBridgeAdapter(
            address(bridge),
            address(verifier),
            admin
        );
    ...
    function test_constructor_setsBridge() public view {
        assertEq(address(adapter.algorandBridge()), address(bridge));
    }

    function test_constructor_setsVerifier() public view {
        assertEq(address(adapter.stateProofVerifier()), address(verifier));
    }
    ...
    function test_constructor_revertsZeroBridge() public {
        vm.expectRevert(AlgorandBridgeAdapter.InvalidBridge.selector);
        new AlgorandBridgeAdapter(address(0), address(verifier), admin);
    }

    function test_constructor_revertsZeroVerifier() public {
        vm.expectRevert(AlgorandBridgeAdapter.InvalidVerifier.selector);
        new AlgorandBridgeAdapter(address(bridge), address(0), admin);
    }

    function test_constructor_revertsZeroAdmin() public {
        vm.expectRevert(AlgorandBridgeAdapter.InvalidTarget.selector);
        new AlgorandBridgeAdapter(
            address(bridge),
            address(verifier),
            address(0)
        );
    ...
    function test_constants() public view {
        assertEq(adapter.ALGORAND_CHAIN_ID(), 26_100);
    ...
    function test_chainId() public view {
        assertEq(adapter.chainId(), 26_100);
    }

    function test_chainName() public view {
        assertEq(adapter.chainName(), "Algorand");
    }
    ...
    function test_sendMessage_revertsEmptyPayload() public {
        vm.prank(operator);
        vm.expectRevert(AlgorandBridgeAdapter.InvalidPayload.selector);
    ...
    function test_receiveMessage_success() public {
        ...
        inputs[0] = uint256(keccak256("participation"));
    ...
    function test_receiveMessage_revertsInvalidProof() public {
        ...
        vm.prank(relayer);
        vm.expectRevert(AlgorandBridgeAdapter.InvalidProof.selector);
    ...
    function test_receiveMessage_revertsDuplicateNullifier() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                AlgorandBridgeAdapter.NullifierAlreadyUsed.selector,
                keccak256("null-dup")
            )
        );
        adapter.receiveMessage(proof, inputs, hex"beef");
    ...
    function test_setBridgeFee_revertsAboveMax() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                AlgorandBridgeAdapter.FeeTooHigh.selector,
                101
            )
        );
