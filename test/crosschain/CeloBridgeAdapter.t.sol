...
import {CeloBridgeAdapter, ICeloBridge, ICeloLightClient} from "../../contracts/crosschain/CeloBridgeAdapter.sol";
import "@openzeppelin/contracts/token/ERC20/ERC20.sol";

contract MockCeloBridge {
    bytes32 public nextMessageId = keccak256("celo-msg-1");
    uint256 public relayFee = 0.001 ether;
    uint256 public verifiedBlock = 20_000_000;
    ...
    function relayToCelo(
        ...
        require(!shouldRevert, "MockCeloBridge: reverted");
...
contract MockCeloLightClient {
    bool public shouldVerify = true;
    bytes32 public validatorSetHash = keccak256("celo-validators-1");
...
contract MockERC20Celo is ERC20 {
...
contract CeloBridgeAdapterTest is Test {
    CeloBridgeAdapter public adapter;
    MockCeloBridge public bridge;
    MockCeloLightClient public lightClient;
    MockERC20Celo public token;
    ...
    function setUp() public {
        bridge = new MockCeloBridge();
        lightClient = new MockCeloLightClient();
        token = new MockERC20Celo();
        adapter = new CeloBridgeAdapter(
            address(bridge),
            address(lightClient),
            admin
        );
    ...
    function test_constructor_setsBridge() public view {
        assertEq(address(adapter.celoBridge()), address(bridge));
    }

    function test_constructor_setsVerifier() public view {
        assertEq(address(adapter.lightClient()), address(lightClient));
    }
    ...
    function test_constructor_revertsZeroBridge() public {
        vm.expectRevert(CeloBridgeAdapter.InvalidBridge.selector);
        new CeloBridgeAdapter(address(0), address(lightClient), admin);
    }

    function test_constructor_revertsZeroVerifier() public {
        vm.expectRevert(CeloBridgeAdapter.InvalidVerifier.selector);
        new CeloBridgeAdapter(address(bridge), address(0), admin);
    }

    function test_constructor_revertsZeroAdmin() public {
        vm.expectRevert(CeloBridgeAdapter.InvalidTarget.selector);
        new CeloBridgeAdapter(
            address(bridge),
            address(lightClient),
            address(0)
        );
    ...
    function test_constants() public view {
        assertEq(adapter.CELO_CHAIN_ID(), 21_100);
    ...
    function test_chainId() public view {
        assertEq(adapter.chainId(), 21_100);
    }

    function test_chainName() public view {
        assertEq(adapter.chainName(), "Celo");
    }
    ...
    function test_sendMessage_revertsEmptyPayload() public {
        vm.prank(operator);
        vm.expectRevert(CeloBridgeAdapter.InvalidPayload.selector);
    ...
    function test_receiveMessage_revertsInvalidProof() public {
        ...
        vm.prank(relayer);
        vm.expectRevert(CeloBridgeAdapter.InvalidProof.selector);
    ...
    function test_receiveMessage_revertsDuplicateNullifier() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                CeloBridgeAdapter.NullifierAlreadyUsed.selector,
                keccak256("null-dup")
            )
        );
        adapter.receiveMessage(proof, inputs, hex"beef");
    ...
    function test_setBridgeFee_revertsAboveMax() public {
        vm.prank(admin);
        vm.expectRevert(
            abi.encodeWithSelector(CeloBridgeAdapter.FeeTooHigh.selector, 101)
        );
    ...
    function testFuzz_setBridgeFee_bounds(uint256 fee) public {
        if (fee <= 100) {
            ...
            vm.expectRevert(
                abi.encodeWithSelector(
                    CeloBridgeAdapter.FeeTooHigh.selector,
                    fee
                )
            );
            adapter.setBridgeFee(fee);
