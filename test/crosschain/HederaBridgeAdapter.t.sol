...
import {HederaBridgeAdapter, IHederaBridge, IHashgraphVerifier} from "../../contracts/crosschain/HederaBridgeAdapter.sol";
import "@openzeppelin/contracts/token/ERC20/ERC20.sol";

contract MockHederaBridge {
    bytes32 public nextMessageId = keccak256("hedera-msg-1");
    uint256 public relayFee = 0.001 ether;
    uint256 public verifiedTimestamp = 1700000000;
    ...
    function relayToHedera(
        ...
        require(!shouldRevert, "MockHederaBridge: reverted");
    ...
    function latestVerifiedTimestamp() external view returns (uint256) {
...
contract MockHashgraphVerifier {
    bool public shouldVerify = true;
    bytes32 public nodeSetHash = keccak256("hedera-nodes-1");
...
contract MockERC20Hedera is ERC20 {
...
contract HederaBridgeAdapterTest is Test {
    HederaBridgeAdapter public adapter;
    MockHederaBridge public bridge;
    MockHashgraphVerifier public verifier;
    MockERC20Hedera public token;
    ...
    function setUp() public {
        bridge = new MockHederaBridge();
        verifier = new MockHashgraphVerifier();
        token = new MockERC20Hedera();
        adapter = new HederaBridgeAdapter(
            address(bridge),
            address(verifier),
            admin
        );
    ...
    function test_constructor_setsBridge() public view {
        assertEq(address(adapter.hederaBridge()), address(bridge));
    }

    function test_constructor_setsVerifier() public view {
        assertEq(address(adapter.hashgraphVerifier()), address(verifier));
    }
    ...
    function test_constructor_revertsZeroBridge() public {
        vm.expectRevert(HederaBridgeAdapter.InvalidBridge.selector);
        new HederaBridgeAdapter(address(0), address(verifier), admin);
    }

    function test_constructor_revertsZeroVerifier() public {
        vm.expectRevert(HederaBridgeAdapter.InvalidVerifier.selector);
        new HederaBridgeAdapter(address(bridge), address(0), admin);
    }

    function test_constructor_revertsZeroAdmin() public {
        vm.expectRevert(HederaBridgeAdapter.InvalidTarget.selector);
        new HederaBridgeAdapter(address(bridge), address(verifier), address(0));
    }

    function test_constants() public view {
        assertEq(adapter.HEDERA_CHAIN_ID(), 25_100);
    ...
    function test_chainId() public view {
        assertEq(adapter.chainId(), 25_100);
    }

    function test_chainName() public view {
        assertEq(adapter.chainName(), "Hedera");
    }
    ...
    function test_sendMessage_revertsEmptyPayload() public {
        vm.prank(operator);
        vm.expectRevert(HederaBridgeAdapter.InvalidPayload.selector);
    ...
    function test_receiveMessage_success() public {
        ...
        inputs[0] = uint256(keccak256("nodeSet"));
    ...
    function test_receiveMessage_revertsInvalidProof() public {
        ...
        vm.prank(relayer);
        vm.expectRevert(HederaBridgeAdapter.InvalidProof.selector);
    ...
    function test_receiveMessage_revertsDuplicateNullifier() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                HederaBridgeAdapter.NullifierAlreadyUsed.selector,
                keccak256("null-dup")
            )
        );
        adapter.receiveMessage(proof, inputs, hex"beef");
    ...
    function test_setBridgeFee_revertsAboveMax() public {
        vm.prank(admin);
        vm.expectRevert(
            abi.encodeWithSelector(HederaBridgeAdapter.FeeTooHigh.selector, 101)
        );
    ...
    function testFuzz_setBridgeFee_bounds(uint256 fee) public {
        if (fee <= 100) {
            ...
            vm.expectRevert(
                abi.encodeWithSelector(
                    HederaBridgeAdapter.FeeTooHigh.selector,
                    fee
                )
            );
            adapter.setBridgeFee(fee);
