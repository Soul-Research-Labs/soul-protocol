...
import {OasisBridgeAdapter, IOasisBridge, IOasisProofVerifier} from "../../contracts/crosschain/OasisBridgeAdapter.sol";
import "@openzeppelin/contracts/token/ERC20/ERC20.sol";

contract MockOasisBridge {
    bytes32 public nextMessageId = keccak256("oasis-msg-1");
    uint256 public relayFee = 0.001 ether;
    uint256 public verifiedBlock = 15_000_000;
    ...
    function relayToOasis(
        ...
        require(!shouldRevert, "MockOasisBridge: reverted");
...
contract MockOasisProofVerifier {
    bool public shouldVerify = true;
    bytes32 public committeeHash = keccak256("oasis-committee-1");
...
contract MockERC20Oasis is ERC20 {
...
contract OasisBridgeAdapterTest is Test {
    OasisBridgeAdapter public adapter;
    MockOasisBridge public bridge;
    MockOasisProofVerifier public verifier;
    MockERC20Oasis public token;
    ...
    function setUp() public {
        bridge = new MockOasisBridge();
        verifier = new MockOasisProofVerifier();
        token = new MockERC20Oasis();
        adapter = new OasisBridgeAdapter(
            address(bridge),
            address(verifier),
            admin
        );
    ...
    function test_constructor_setsBridge() public view {
        assertEq(address(adapter.oasisBridge()), address(bridge));
    }
    ...
    function test_constructor_revertsZeroBridge() public {
        vm.expectRevert(OasisBridgeAdapter.InvalidBridge.selector);
        new OasisBridgeAdapter(address(0), address(verifier), admin);
    }

    function test_constructor_revertsZeroVerifier() public {
        vm.expectRevert(OasisBridgeAdapter.InvalidVerifier.selector);
        new OasisBridgeAdapter(address(bridge), address(0), admin);
    }

    function test_constructor_revertsZeroAdmin() public {
        vm.expectRevert(OasisBridgeAdapter.InvalidTarget.selector);
        new OasisBridgeAdapter(address(bridge), address(verifier), address(0));
    }

    function test_constants() public view {
        assertEq(adapter.OASIS_CHAIN_ID(), 24_100);
    ...
    function test_chainId() public view {
        assertEq(adapter.chainId(), 24_100);
    }

    function test_chainName() public view {
        assertEq(adapter.chainName(), "Oasis Sapphire");
    }
    ...
    function test_sendMessage_revertsEmptyPayload() public {
        vm.prank(operator);
        vm.expectRevert(OasisBridgeAdapter.InvalidPayload.selector);
    ...
    function test_receiveMessage_success() public {
        ...
        inputs[0] = uint256(keccak256("committee"));
    ...
    function test_receiveMessage_revertsInvalidProof() public {
        ...
        vm.prank(relayer);
        vm.expectRevert(OasisBridgeAdapter.InvalidProof.selector);
    ...
    function test_receiveMessage_revertsDuplicateNullifier() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                OasisBridgeAdapter.NullifierAlreadyUsed.selector,
                keccak256("null-dup")
            )
        );
        adapter.receiveMessage(proof, inputs, hex"beef");
    ...
    function test_setBridgeFee_revertsAboveMax() public {
        vm.prank(admin);
        vm.expectRevert(
            abi.encodeWithSelector(OasisBridgeAdapter.FeeTooHigh.selector, 101)
        );
    ...
    function testFuzz_setBridgeFee_bounds(uint256 fee) public {
        if (fee <= 100) {
            ...
            vm.expectRevert(
                abi.encodeWithSelector(
                    OasisBridgeAdapter.FeeTooHigh.selector,
                    fee
                )
            );
            adapter.setBridgeFee(fee);
