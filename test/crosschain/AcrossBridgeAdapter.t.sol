...
import {AcrossBridgeAdapter, IAcrossSpokePool, IAcrossHubPool} from "../../contracts/crosschain/AcrossBridgeAdapter.sol";
...
contract MockAcrossSpokePool {
    ...
    function depositV3(
        ...
        require(!shouldRevert, "MockAcrossSpokePool: reverted");
    }
...
contract MockAcrossHubPool {
    ...
    bytes32 public relayerPoolHash = keccak256("across-relayers-1");
...
contract MockERC20Across is ERC20 {
...
contract AcrossBridgeAdapterTest is Test {
    AcrossBridgeAdapter public adapter;
    MockAcrossSpokePool public bridge;
    MockAcrossHubPool public verifier;
    MockERC20Across public token;
    ...
    function setUp() public {
        bridge = new MockAcrossSpokePool();
        verifier = new MockAcrossHubPool();
        token = new MockERC20Across();
    ...
    function test_constructor_setsBridge() public view {
        assertEq(address(adapter.spokePool()), address(bridge));
    }

    function test_constructor_setsVerifier() public view {
        assertEq(address(adapter.hubPool()), address(verifier));
    }
    ...
    function test_constructor_revertsZeroBridge() public {
        vm.expectRevert(AcrossBridgeAdapter.InvalidBridge.selector);
        new AcrossBridgeAdapter(address(0), address(verifier), admin);
    }

    function test_constructor_revertsZeroVerifier() public {
        vm.expectRevert(AcrossBridgeAdapter.InvalidVerifier.selector);
        new AcrossBridgeAdapter(address(bridge), address(0), admin);
    }

    function test_constructor_revertsZeroAdmin() public {
        vm.expectRevert(AcrossBridgeAdapter.InvalidTarget.selector);
        new AcrossBridgeAdapter(address(bridge), address(verifier), address(0));
    }

    function test_constants() public view {
        assertEq(adapter.ACROSS_PROTOCOL_ID(), 28_100);
    ...
    function test_chainId() public view {
        assertEq(adapter.chainId(), 28_100);
    }

    function test_chainName() public view {
        assertEq(adapter.chainName(), "Across");
    }
    ...
    function test_sendMessage_revertsEmptyPayload() public {
        vm.prank(operator);
        vm.expectRevert(AcrossBridgeAdapter.InvalidPayload.selector);
    ...
    function test_receiveMessage_success() public {
        ...
        inputs[0] = uint256(keccak256("relayerPool"));
    ...
    function test_receiveMessage_revertsInvalidProof() public {
        ...
        vm.prank(relayer);
        vm.expectRevert(AcrossBridgeAdapter.InvalidProof.selector);
    ...
    function test_receiveMessage_revertsDuplicateNullifier() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                AcrossBridgeAdapter.NullifierAlreadyUsed.selector,
                keccak256("null-dup")
            )
        );
        adapter.receiveMessage(proof, inputs, hex"beef");
    ...
    function test_setBridgeFee_revertsAboveMax() public {
        vm.prank(admin);
        vm.expectRevert(
            abi.encodeWithSelector(AcrossBridgeAdapter.FeeTooHigh.selector, 101)
        );
    ...
    function testFuzz_setBridgeFee_bounds(uint256 fee) public {
        if (fee <= 100) {
            ...
            vm.expectRevert(
                abi.encodeWithSelector(
                    AcrossBridgeAdapter.FeeTooHigh.selector,
                    fee
                )
            );
            adapter.setBridgeFee(fee);
