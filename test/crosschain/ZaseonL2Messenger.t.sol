...
import "../../contracts/crosschain/ZaseonL2Messenger.sol";
import {IZaseonL2Messenger} from "../../contracts/interfaces/IZaseonL2Messenger.sol";
...
contract ZaseonL2MessengerTest is Test {
    ZaseonL2Messenger public messenger;
    ...
    function setUp() public {
        messenger = new ZaseonL2Messenger(proofHub);
    ...
    function test_sendPrivacyMessage_emitsEvent() public {
        ...
        emit IZaseonL2Messenger.PrivacyMessageSent(
    ...
    function test_sendPrivacyMessage_revertOnInvalidDest() public {
        vm.prank(user);
        vm.expectRevert(ZaseonL2Messenger.InvalidDestinationChain.selector);
        messenger.sendPrivacyMessage(
    ...
    function test_sendPrivacyMessage_revertOnUsedNullifier() public {
        ...
        vm.prank(user);
        vm.expectRevert(ZaseonL2Messenger.NullifierAlreadyUsed.selector);
        messenger.sendPrivacyMessage(
            DEST_CHAIN,
            address(target),
            "",
    ...
    function test_requestL2Call_basic() public {
        IZaseonL2Messenger.Call[] memory calls = new IZaseonL2Messenger.Call[](1);
        calls[0] = IZaseonL2Messenger.Call({
            to: address(target),
            data: abi.encodeWithSelector(MockTarget.setData.selector, 42),
            value: 0
        });

        IZaseonL2Messenger.CrossL2Request memory request = IZaseonL2Messenger
            .CrossL2Request({
                calls: calls,
    ...
    function test_requestL2Call_revertOnEmptyCalls() public {
        IZaseonL2Messenger.Call[] memory calls = new IZaseonL2Messenger.Call[](0);
        ...
        vm.prank(user);
        vm.expectRevert(ZaseonL2Messenger.ExecutionFailed.selector);
    ...
    function test_requestL2Call_revertOnInsufficientValue() public {
        ...
        vm.prank(user);
        vm.expectRevert(ZaseonL2Messenger.InsufficientValue.selector);
    ...
    function test_fulfillMessage_revertOnInsufficientBond() public {
        ...
        vm.prank(fulfiller2);
        vm.expectRevert(ZaseonL2Messenger.InsufficientBond.selector);
        messenger.fulfillMessage(msgId, calldata_, "");
    }

    function test_fulfillMessage_revertOnMessageNotFound() public {
        vm.prank(fulfiller1);
        vm.expectRevert(ZaseonL2Messenger.MessageNotFound.selector);
    ...
    function test_fulfillMessage_revertOnExpired() public {
        ...
        vm.prank(fulfiller1);
        vm.expectRevert(ZaseonL2Messenger.MessageExpired.selector);
        messenger.fulfillMessage(msgId, calldata_, "");
    }
    ...
    function test_receiveMessage_revertOnUnauthorized() public {
        vm.prank(user);
        vm.expectRevert(ZaseonL2Messenger.InvalidCounterpart.selector);
        messenger.receiveMessage(
            SOURCE_CHAIN,
            keccak256("bad"),
            address(target),
    ...
    function test_receiveMessage_failedExecution() public {
        ...
        emit IZaseonL2Messenger.PrivacyMessageFailed(
            keccak256("fail_recv"),
            "Execution failed"
        );
        messenger.receiveMessage(
