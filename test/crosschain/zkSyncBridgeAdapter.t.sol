...
contract zkSyncBridgeAdapterTest is Test {
    ...
    function setUp() public {
        adapter = new zkSyncBridgeAdapter(admin);
        vm.startPrank(admin);
    ...
    function test_ConfigureBridge() public {
        vm.prank(operator);
        adapter.configureBridge(
            324, // zkSync Era chain ID
            address(0x1), // diamondProxy
            address(0x2), // l1Bridge
            address(0x3) // l2Bridge
        );
    }

    function test_ConfigureBridge_RevertNonOperator() public {
        vm.prank(user);
        vm.expectRevert();
        adapter.configureBridge(324, address(0x1), address(0x2), address(0x3));
    }

    function test_MapToken() public {
        vm.prank(operator);
        adapter.mapToken(address(0xDA1), address(0xDA2), 324, 18);
    }
    ...
    function test_RegisterWithdrawal_RevertNonExecutor() public {
        vm.prank(user);
        vm.expectRevert();
        adapter.registerWithdrawal(user, user, address(0), 1 ether, 1, 0, 0);
    }
    ...
    function test_BridgeMessage_RevertsUnconfiguredChain() public {
        vm.deal(user, 1 ether);
        vm.prank(user);
        // Payload encodes chainId=999 (unconfigured) + message
        bytes memory payload = abi.encodePacked(uint256(999), bytes("hello"));
        vm.expectRevert();
        adapter.bridgeMessage{value: 0.5 ether}(
            address(0x1),
            payload,
            address(0)
        );
    ...
    function test_EstimateFee_Reverts() public {
        vm.expectRevert("Use zkSync-specific fee estimation");
